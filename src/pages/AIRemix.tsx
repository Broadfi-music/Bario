  ...
  const featureCards = [
    {
      ...
      description:
        'Transform any song into multiple genres instantly with our advanced AI technology. From amapiano to trap, your music adapts to any style.',
      image: card1,
    },
    {
      id: 2,
      ...
      description:
        'Get your remixed tracks in seconds, not hours. Our powerful processing engine delivers studio-quality results instantly.',
      image: card2,
    },
    {
      id: 3,
      ...
      description:
        "Experience high-fidelity audio that rivals professional studio productions. Every remix maintains the original's clarity and depth.",
      image: card3,
    },
    {
      id: 4,
      ...
      description:
        'Experiment with countless genres and styles. Create unlimited variations until you find the perfect sound for your vision.',
      image: card4,
    },
  ];
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
                    <div
                      className={`rounded-full p-3 sm:p-4 transition-all duration-200 ${
                        playingTrack === track.id ? 'bg-foreground/90 scale-110' : 'bg-foreground/80'
                      }`}
                    >
                      {playingTrack === track.id ? (
                        <Pause className="h-6 w-6 sm:h-8 sm:w-8 text-background fill-background" />
                      ) : (
                        <Play className="h-6 w-6 sm:h-8 sm:w-8 text-background fill-background" />
                      )}
                    </div>
                  </div>
                </button>
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            {featureCards.map((card) => (
              <div
                key={card.id}
                className="bg-black rounded-2xl p-4 sm:p-6 flex flex-col items-start space-y-4 hover:scale-[1.02] transition-transform duration-200"
              >
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-foreground/70 text-xs sm:text-sm leading-relaxed">{card.description}</p>
                </div>
              </div>
            ))}
          </div>
